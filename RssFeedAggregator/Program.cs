...
v1.MapGet("/newest-posts", GetNewestPostsHandler.Execute).WithAuth();


// Set up background tasks
var handler = sp.GetRequiredService<FetchFeedsHandler>();
var recurringJobs = sp.GetRequiredService<IRecurringJobManager>();
//recurringJobs.AddOrUpdate("sync-feeds-sequentially", () => handler.FetchFeedsSequentially(), Cron.Minutely);
//recurringJobs.AddOrUpdate("sync-feeds-concurrently", () => handler.FetchFeedsConcurrently(), Cron.Minutely);
//recurringJobs.AddOrUpdate("100x", () => FetchFeedsHandler.DoSomething100Times(), Cron.Minutely);
